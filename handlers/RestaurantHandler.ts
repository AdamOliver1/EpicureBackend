  ...
  implements IRestaurantHandler
{
  constructor(
    ...
    protected readonly restaurantRepository: IRestaurantRepository,
  ...
  async getLimited(limit: number): Promise<IRestaurant[]> {
    return await this.restaurantRepository.getLimited(3);
  }

  async getByChef(chefId: string): Promise<IRestaurant[]> {
    return await this.restaurantRepository.getByChef(chefId);
  }

  async disable(id: string): Promise<any> {
    const restaurant = await this.restaurantRepository.findById(id);
    if (restaurant?._id) {
      const dishes = await this.dishRepository.getDishesByRestaurant(
        restaurant?._id
      );
      dishes.forEach((d) => {
        if (d._id) this.dishRepository.Disable(d._id);
      });
